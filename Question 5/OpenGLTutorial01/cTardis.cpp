...
void cTardis::Update()
{
	...
	this->getTardisPosition(myPos);
	float newDistToGoal = glm::distance(myPos, this->endPosition);
	if (newDistToGoal < 1.5f)
		this->isEscaped = true;
...
DWORD WINAPI TardisThread(void* TardisData)
{
	...
	if (theTardis->isEscaped)
	{
		std::cout << "WOW. THE DOCTOR HAS ESCAPED." << std::endl;
	}

	else if (theTardis->isCaptured)
	{
		std::cout << "THE DOCTOR IS DEAD. LONG LIVE THE DALEKS." << std::endl;
	}
