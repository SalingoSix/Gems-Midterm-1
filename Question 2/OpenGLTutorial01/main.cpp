...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//Also Z is 20 larger than the circle's centre so we can look down from an angle. Adjust that too
		glm::vec3 adjustedCamera = glm::vec3(Camera.position.x, 0.0f, Camera.position.z - 20.0f);
